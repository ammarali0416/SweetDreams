...
// to add: add a threadid variable to be sent to the server. It should start out as null, and be populated in the response by the server.

const SweetDreams = () => {
  const [message, setMessage] = useState('');
  const [chatHistory, setChatHistory] = useState([]);

  const summaryComplete = false; // flag to check if the conversation with the editor in chief is complete
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { reply: botReply, summaryComplete } = responseData;
